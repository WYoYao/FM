...
// 2017.01.08 add start
function showSystListLike(dom) {
    var $sarrow = $(dom);
    var $buildTitle = $sarrow.parent(".buildTitle");
    if ($buildTitle.attr('stype') == 'show') { //展开状态
        $buildTitle.siblings(".systemCont").slideUp();
...
function showSystemCont(){
    $(".buildItem:visible").find(".buildTitle[stype='close']").each(function () {
        var $sarrow=$(this).children(".arrow");
        showSystListLike($sarrow)
    })
}
// 2017.01.08 add end
    ...
    return function (item) {
        ...
        if(key=="domin" || key=="system_category"){
            ...
            if(key == "domin"){
                ...
                v.instance.InsertSystemModel.system_category = "";
                v.instance.SystemScrollList = [{
                    title: "基础",
                    id: 'base',
                    isSelected: true,
                    top: 60,
