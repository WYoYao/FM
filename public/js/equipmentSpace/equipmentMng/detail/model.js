    ...
    v.pushComponent({
        ...
        methods: {
            ...
            showPartIndex: function (align, index) {
                ...
                var dis = el.find(".part").eq(index)[0].offsetTop;
                el.find(".detail2_contentView:visible")[0].scrollTop = dis;
            ...
            _clickIfmsSubmit: function (event, info) {
                ...
                window.activeElementObj = info;
                window.activeElementVal = $(event.target).parent().parent().parent().find(".editInput>div").psel().text;
                ...
                submitCb = function (isNewValue) { 
                    ...
                    equipmentMngDeatilController.updateEquipInfo(req, (type == 3))
                        .then(function (data) {
                            ...
                            activeElementObj.isShow = false;
                            //判断textarea类型重新赋值
                            // if(activeElementObj.cmpt == 'ptext_textarea'){
                            //     activeElementObj.str_value = activeElementObj.name;
                            // }
                            if(activeElementObj.str_arr_value && (activeElementObj.str_arr_value.length > 0)){ // 区间

                                activeElementObj.str_value[0] = activeElementObj.str_arr_value[0];
                                activeElementObj.str_value[1] = activeElementObj.str_arr_value[1];
                                activeElementObj.name = activeElementObj.str_arr_value[0] + info.unit + "~" + activeElementObj.str_arr_value[1] + info.unit; 

                            }else if(activeElementObj.cmpt_data && (activeElementObj.cmpt_data.length > 0)){ // 单选

                                for(var i=0;i<activeElementObj.cmpt_data.length;i++){
                                    activeElementObj.cmpt_data[i].isSelected = false;
                                    if(activeElementObj.cmpt_data[i].name == activeElementVal){

                                        activeElementObj.name = activeElementObj.cmpt_data[i];
                                        activeElementObj.cmpt_data[i].isSelected = true;
                                        activeElementObj.str_value = activeElementObj.cmpt_data[i].code;
                                ...
                                //文本域类型
                                if(activeElementObj.cmpt == 'ptext_textarea'){
                                    activeElementObj.name =  activeElementObj.str_value;
                                }else{
                                    activeElementObj.name = activeElementObj.str_value+info.unit;
                                }
            ...
            _clickIfmsCancel: function (event, info) {
                ...
                var cancelCb = function () {
                    ...
                    info.isShow = false;
                    // _that.requeryEquipDynamicInfo();
                    if(info.str_arr_value && (info.str_arr_value.length > 0)){ // 区间
                        info.str_arr_value[0] = info.str_value[0];
                        info.str_arr_value[1] = info.str_value[1];
                        info.name = info.str_arr_value[0] + info.unit + "~" + info.str_arr_value[1] + info.unit; 

                    }else if(info.cmpt_data && (info.cmpt_data.length > 0)){

                    }else if(info.unit && (info.unit.length > 0)){  // 字符串
                        info.str_value = info.name.slice(0,(info.name.length - info.unit.length));
                    } 
                                ...
                                if (key == "manufacturer") {
                                    ...
                                    equipmentMngDeatilController.updateEquipInfo({
                                        ...
                                        _that.requeryEquipPublicInfo(function () {
                                            ...
                                            // _that._clickStartChange("brand");  //编辑设备品牌
                                        });
                                    })
             ...
             * 新增部分 转换为可以提交的参数
             */
            convert2Controller: function (item, key) {
            ...
            getReqByKey: function (item) {
                ...
                if (type == 0) {
                    ...
                    req = this.convert2Controller(Object.assign({}, info, {
                        key: key
                    }), ['manufacturer_id', 'maintainer_id', 'supplier_id', 'insurer_id'].indexOf(key) != -1 ? "company_id" : void 0);
            ...
            _clickAddinsurance: function () {
                ...
                var insuranceArr = this.getSettByKey(['insurer_id', 'insurer_num']);
            ...
            _clickAddbuy: function () {
                ...
                ].concat(this.getSettByKey('supplier_id'));
            ...
            _clickAddfactory: function () {
                //判断出厂编号不能超过字数限制
                var self = this;
                var factoryNo = $("#addid_serial_num").pval();
                var equie = $("#addid_brand").pval();
                var factoryArr = [{
                    key: 'product_date'
                },{
                    key: 'serial_num'
                    ...
                    key: 'brand'
                }].concat(this.getSettByKey(['manufacturer_id', 'specification']));

                if(factoryNo !='' && equie == ''){
                    if($("#addid_serial_num").pverifi()){
                        this.someSend(factoryArr);
                    }else{
                        self.view.add.factory.isSHowAddBlock = false;
                        return;
                    }
                } else if(factoryNo =='' && equie != ''){
                    if($("#addid_brand").pverifi()){
                        this.someSend(factoryArr);
                    }else{
                        self.view.add.factory.isSHowAddBlock = false;
                        return;
                    ...
                    if($("#addid_serial_num").pverifi() && $("#addid_brand").pverifi()){
                        this.someSend(factoryArr);
                    }else{
                        self.view.add.factory.isSHowAddBlock = false;
                        return;
            ...
            _clickAddMaintenance: function () {
                ...
                var serviceLife = $("#addid_service_life").pval(),
                    warranty = $("#addid_warranty").pval()
                if (serviceLife && warranty) {
                ...
                ].concat(this.getSettByKey(['maintainer_id']));
            ...
            requeryEquipPublicInfo: function (cb) {
                ...
                PromsQueryEquipPublicInfo.then(function (info) {
                    ...
                    _that.equip_models = brs.length ? brs[0].equip_models.map(function (i) {
        ...
        beforeMount: function () {
            ...
            proQueryEquipCompanySels.then(function (list) {
                ...
                $("#addid_specification").pdisable(true);
            })
            ...
            this.$nextTick(function () {
                //设备详情页--基础
                $(".detail2_contentView:visible")[0].addEventListener('scroll', function () {
                    _that.onScrollPub(".detail2_contentView", ".detail2_verticalView", ".detail2_verticalView", ".infos", _that.view.scroll)
                });
            })
        ...
        watch: {
            ...
            equip_models: function (newVal, oldVal) {

                var _that = this;
                if (newVal != oldVal) {
                    $("#addid_specification").precover();
                    $("#addid_specification").pdisable(false)
                }
            },
