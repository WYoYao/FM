...
spaceInfoController.querySpaceRemindConfig = function () { //查询房间提醒设置
    ...
    pajax.post({
        ...
        data: {
          person_id: instance.userInfo.person_id
        },
        success: function (res) {
            data = res.data || [];
...
spaceInfoController.queryFloorIdentity = function () { 
 ...
 //获取用户信息
 spaceInfoController.getUserInfo=function () {
    var instance = spaceInfoController.systemModelObj || spaceInfoModel.instance();
    $.ajax({
        url: '/userInfo',      
        data: {},
        success: function (result) {
            instance.userInfo = result;
        },
    ...
    downDataFile: function () {
        return new Promise(function (resolve, reject) {
            pajax.downloadByParam("restImportPhysicsWorld/downloadEquipTemplateFile", {
                isEquip: "0"   	// 1-下载设备模板exl；0-下载楼层房间模板exl，必须
            });
        })
    },
