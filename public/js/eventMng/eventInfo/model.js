v.pushComponent({
    ...
    methods: {
        ...
        stopModelEvPropagation: function (type) {
            if (type) {
                $(".per-modal-mask").bind('click', function (ev) { ev.stopPropagation() });
            } else {
                $(".per-modal-mask").unbind('click', function (ev) { ev.stopPropagation() });
            }
        },
        // 获取项目事件详情
        getProEvInfo: function () {
            var that = this;
            $("#EvInfoLoading").pshow();
            EMA.EI({ eventId: this.proEvInfoData.easy.eventId }, function (data) {
        ...
        eventToWorkOrder: function (id) {
            var that = this;
            EMA.EI({ eventId: id }, function (data) {
        ...
        editEventInfo: function (type) {
            if (type) {
                ...
                var param = {
                    ...
                    reviseProblemType : $("#editEvTypeCombo").psel().index,
        ...
        getGroupEvInfo : function(){
            var that = this;
            $("#EvInfoLoading").pshow();
            EMA.GE({groupEventId:that.groupEvOpeId},function(data){
                that.groupEvInfoData.full = JSON.parse(JSON.stringify(data)) || {};
